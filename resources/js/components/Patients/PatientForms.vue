<template>
  <div class="sidebar close">
    ...
    <ul class="nav-link">
      ...
      <li><div class="profile-details" @click="confirmLogout">
      ...
      <div class="header">
        ...
        <div class="text">
          ...
          <div class="breadcrumbs">
            ...
            <a class="current" href="/patientforms"><p>Forms</p></a>
          </div>
        </div>
      </div>
      <hr color="#06693a">
      ...
      <div class="search-wrapper">
        <div class="search-container">
          <input 
            ...
            @keyup.enter="filterForms" 
            class="search-input"
            :class="{ 'has-reset': searchQuery && isSearchActive }"
          />
          <i class="bx bx-search search-icon" @click="filterForms"></i>
          <button 
            v-if="isSearchActive" 
            @click="resetSearch" 
            class="reset-button"
        ...
        <div class="table-wrapper">
          ...
          <table class="treatment-table desktop-table">
            ...
            <tbody class="container-treatment">
              <tr 
                v-for="(form, index) in filteredDentalForms" 
                :key="form.id"
                class="treatment-row"
              >
                <td style="padding-left: 25px;" class="truncate-cell">{{ index + 1 }}</td>
                <td class="truncate-cell">{{ form.description }}</td>
                <td class="truncate-cell">
                  <div class="file-type-badge" :class="getFileTypeClass(form.type)">
                    {{ form.type || 'N/A' }}
                  </div>
                </td>
                <td class="truncate-cell">{{ formatDate(form.date) }}</td>
                <td class="action-cell">
                  <button @click="handleFilePreview(form)" class="view-details-btn" title="View Details">
                    <i class='bx bx-show'></i>
                  </button>
                  <button @click="handleFileDownload(form)" class="download-btn" title="Download">
                    <i class='bx bx-download'></i>
                  </button>
                </td>
              </tr>
              <tr v-if="filteredDentalForms.length === 0">
                <td colspan="5" class="no-data">No dental forms found.</td>
              </tr>
            </tbody>
          </table>
          ...
          <div class="mobile-cards">
            ...
            >
              ...
              <div class="card-footer">
                <button @click.stop="handleFilePreview(form)" class="action-button-mobile">
                  <i class='bx bx-show'></i> View
                </button>
                <button @click.stop="handleFileDownload(form)" class="action-button-mobile">
                  <i class='bx bx-download'></i> Download
                </button>
              </div>
            </div>
            <div v-if="filteredDentalForms.length === 0" class="no-data-mobile">
              No dental forms found
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchDentalForms() {
      try {
        const response = await axios.get('/api/patient/dental-forms');
        this.dentalForms = response.data.map(form => ({
          id: form.DentalFormID,
          name: form.FileName,
          type: form.FileType,
          description: form.FileName,
          date: form.created_at, // Updated to match response field
          appointment: form.Appointment ? {
            id: form.Appointment.AppointmentID,
            date: form.Appointment.AppointmentDate,
            time: form.Appointment.AppointmentTime,
            status: form.Appointment.AppointmentStatus,
          } : null,
        }));
        this.filteredDentalForms = [...this.dentalForms];
      } catch (error) {
        console.error('Error fetching dental forms:', error);
        if (error.response && error.response.status === 401) {
          window.location.href = '/login';
        } else if (error.response && error.response.status === 500) {
          alert(error.response.data.message || 'An error occurred on the server. Please try again later.');
        } else {
          alert('Failed to fetch dental forms. Please try again.');
        }
        this.dentalForms = [];
        this.filteredDentalForms = [];
      }
    },
    matchesSearch(form) {
      ...
      return (
        ...
        (this.stripFileExtension(form.name) || '').toLowerCase().includes(query) ||
        (form.type || '').toLowerCase().includes(query) ||
        (this.formatDate(form.date) || '').toLowerCase().includes(query) ||
        (form.appointment?.date || '').toLowerCase().includes(query) ||
        (form.appointment?.status || '').toLowerCase().includes(query)
      );
    },
    applyAllFilters(forms) {
      let filtered = [...forms];
      
      if (this.searchQuery && this.isSearchActive) {
        filtered = filtered.filter(form => this.matchesSearch(form));
      }
      ...
      if (this.filter.startDate || this.filter.endDate) {
        filtered = filtered.filter(form => {
          ...
          return (!start || formDate >= start) && (!end || formDate <= end);
        });
      }
    ...
    updateDisplayedForms() {
      this.filteredDentalForms = this.applyAllFilters(this.dentalForms);
      this.applySorting();
    },
    toggleFilterMenu() {
    ...
    applySorting() {
      ...
      this.filteredDentalForms.sort((a, b) => {
        let valueA = key === 'index' ? this.filteredDentalForms.indexOf(a) + 1 : a[key];
        let valueB = key === 'index' ? this.filteredDentalForms.indexOf(b) + 1 : b[key];
    ...
    async handleFilePreview(file) {
      try {
          const endpoint = `/api/patient/download/form/${file.id}?inline=true`;
          ...
          console.error('Error fetching file for preview:', error, error.response ? error.response.data : error.message);
          ...
          if (error.response && error.response.status === 401) {
              ...
              errorMessage = error.response.data.message || 'An error occurred on the server while previewing the file.';
          }
          this.$notify({
              type: 'error',
    ...
    async handleFileDownload(file) {
      try {
        const endpoint = `/api/patient/download/form/${file.id}`;
        const response = await axios.get(endpoint, { responseType: 'blob' });
        ...
        link.href = url;
        link.download = file.name || `dental_form_${file.id}.${file.type}`;

  ...
  <div class="home-content">
    <div class="header">
      ...
      <div class="text">
          ...
          <div class="breadcrumbs">
            ...
            <a class="current" href="/patienttreatments"><p>  Treatments</p></a>
          </div>
      </div>
    </div>
    <hr color="#06693a">
    ...
    <div class="search-wrapper">
      <div class="search-container">
        <input 
          v-model="searchQuery" 
          placeholder="Search by description, date, or category" 
          @keyup.enter="filterFiles" 
          class="search-input"
          :class="{ 'has-reset': searchQuery && isSearchActive }"
        />
        <i class="bx bx-search search-icon" @click="filterFiles"></i>
        <button 
          v-if="isSearchActive" 
          @click="resetSearch" 
          class="reset-button"
    ...
    <div class="toggle-buttons">
      <button 
        ...
        @click="activeTab = 'new'"
      >
        <i class='bx bx-upload'></i> New Uploaded Files
      </button>
      <button 
        class="toggle-btn" 
      ...
      >
        <i class='bx bx-history'></i> Previous Uploads
      </button>
    </div>
    
    <!-- New Uploaded Files Section -->
    <div class="upload-container" v-show="activeTab === 'new'">
      <div class="upload">
        <h3>NEW UPLOADED FILES</h3>
      </div>
      <div class="table-wrapper">
        <table class="upload-table desktop-table">
          <thead>
            <tr>
              ...
              <th @click="sortTable('dateAdded')" class="sortable-header">
                Date Added <i v-if="sortConfig.key === 'dateAdded'" class="bx" :class="getSortIconClass('dateAdded')"></i>
              </th>
              <th @click="sortTable('category')" class="sortable-header">
                Category <i v-if="sortConfig.key === 'category'" class="bx" :class="getSortIconClass('category')"></i>
              </th>
              <th style="text-align: center;" class="action-header">Action</th>
            </tr>
          </thead>
          <tbody class="container-upload">
            <tr v-for="(file, index) in filteredNewFiles" :key="index" class="upload-row">
              <td style="padding-left: 25px;" class="truncate-cell">{{ index + 1 }}</td>
              <td class="truncate-cell">{{ file.description || 'N/A' }}</td>
              <td class="truncate-cell">{{ formatDate(file.dateAdded) || 'N/A' }}</td>
              <td class="truncate-cell">
                <div class="file-type-badge" :class="getFileTypeClass(file.category)">
                  {{ file.category || 'N/A' }}
                </div>
              </td>
              <td class="action-cell">
                <button @click="handleFileAction(file)" class="view-details-btn" title="View Details">
                  <i class='bx bx-show'></i>
                </button>
                <button @click="handleFileDownload(file)" class="download-btn" title="Download">
                  <i class='bx bx-download'></i>
                </button>
              </td>
            </tr>
            <tr v-if="filteredNewFiles.length === 0">
              <td colspan="5" class="no-data">No New Uploaded Files...</td>
            </tr>
          </tbody>
        </table>
        <!-- Mobile Cards -->
        <div class="mobile-cards">
          <div v-for="(file, index) in filteredNewFiles" :key="index" class="upload-card">
            <div class="card-header">
              <h4>{{ file.description || 'N/A' }}</h4>
              <div class="file-type-badge" :class="getFileTypeClass(file.category)">
                {{ file.category || 'N/A' }}
              </div>
            </div>
            <div class="card-content">
              ...
              <div class="card-row">
                ...
                <span>{{ formatDate(file.dateAdded) || 'N/A' }}</span>
              </div>
            </div>
            <div class="card-footer">
              <button @click.stop="handleFileAction(file)" class="action-button-mobile">
                <i class='bx bx-show'></i> View
              </button>
              <button @click.stop="handleFileDownload(file)" class="action-button-mobile">
                <i class='bx bx-download'></i> Download
              </button>
            </div>
          </div>
          <div v-if="filteredNewFiles.length === 0" class="no-data-mobile">
            No New Uploaded Files
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="upload-container" v-show="activeTab === 'previous'">
      <div class="upload">
      ...
      <div class="table-wrapper">
        <table class="upload-table desktop-table">
          ...
          <tbody class="container-upload">
            <tr v-for="(file, index) in filteredPreviousFiles" :key="index" class="upload-row">
              <td style="padding-left: 25px;" class="truncate-cell">{{ index + 1 }}</td>
              <td class="truncate-cell">{{ file.description || 'N/A' }}</td>
              <td class="truncate-cell">{{ formatDate(file.dateAdded) || 'N/A' }}</td>
              <td class="truncate-cell">
            ...
            <tr v-if="filteredPreviousFiles.length === 0">
              <td colspan="5" class="no-data">No Previous Uploads...</td>
            </tr>
          </tbody>
        </table>
        <!-- Mobile Cards -->
        <div class="mobile-cards">
          <div v-for="(file, index) in filteredPreviousFiles" :key="index" class="upload-card">
            <div class="card-header">
              <h4>{{ file.description || 'N/A' }}</h4>
              <div class="file-type-badge" :class="getFileTypeClass(file.category)">
                {{ file.category || 'N/A' }}
          ...
          <div v-if="filteredPreviousFiles.length === 0" class="no-data-mobile">
            No Previous Uploads
          </div>
        </div>
      </div>
    ...
    <!-- Preview Modal - Back to Original Design -->
    <div v-if="showPreviewModal" class="modal-overlay" @click="closePreviewModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>{{ previewFile?.description || 'File Preview' }}</h3>
        ...
        <div class="modal-body">
          <div class="file-info">
            <div class="file-type-badge large" :class="getFileTypeClass(previewFile?.category)">
              {{ previewFile?.category }}
            </div>
            <div class="file-details">
              <p><strong>Date Added:</strong> {{ formatDate(previewFile?.dateAdded) }}</p>
              <p v-if="previewFile?.name"><strong>Filename:</strong> {{ previewFile?.name }}</p>
              <p v-if="previewFile?.type"><strong>File Type:</strong> {{ previewFile?.type }}</p>
            </div>
          </div>
          <div class="preview-content">
            ...
            <div v-else-if="previewFile?.category === 'Treatment History'" class="treatment-history-details">
              <div class="details-row">
                <span class="details-label">Treatment:</span>
                <span>{{ previewFile?.treatment || 'N/A' }}</span>
              </div>
              <div class="details-row">
                <span class="details-label">Dentist:</span>
                <span>{{ previewFile?.dentist || 'N/A' }}</span>
              </div>
              <div class="details-row" v-if="previewFile?.notes">
                <span class="details-label">Notes:</span>
                <p class="details-notes">{{ previewFile?.notes }}</p>
              </div>
            </div>
            <div v-else class="preview-unavailable">
              <i class='bx bx-file'></i>
              <p>Preview not available for this file type.</p>
              <a v-if="previewFile?.url" :href="previewFile.url" download class="download-button">
...
export default {
  ...
  computed: {
    ...
    uniqueCategories() {
      const allFiles = [...this.allNewFiles, ...this.allPreviousFiles];
      return [...new Set(allFiles.map(file => file.category).filter(Boolean))];
    }
  },
  mounted() {
    this.fetchAllFiles();
  ...
  methods: {
    ...
    async fetchAllFiles() {
      try {
        ...
        // Fetch all types of files in parallel
        const [treatmentPlans, dentalPhotos, forms, treatmentHistory] = await Promise.all([
          this.fetchTreatmentPlans(),
          this.fetchDentalPhotos(),
          this.fetchForms(),
        ...
        const allFiles = [
          ...treatmentPlans.map(file => ({
            ...file,
            category: 'Treatment Plan',
            sourceType: 'treatment-plan'
          })),
          ...dentalPhotos.map(file => ({
            ...file,
            category: 'Dental Photos',
            sourceType: 'dental-photos'
          })),
          ...forms.map(file => ({
            ...file,
            category: 'Forms',
          ...
          ...treatmentHistory.map(file => ({
            ...file,
            category: 'Treatment History',
            sourceType: 'treatment-history'
          }))
        ];
        
        // Sort all files by date (newest first)
        allFiles.sort((a, b) => new Date(b.dateAdded) - new Date(a.dateAdded));
        
        // Separate files into new (today) and previous
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        
        this.allNewFiles = allFiles.filter(file => {
          const fileDate = new Date(file.dateAdded);
          fileDate.setHours(0, 0, 0, 0);
          return fileDate.getTime() === today.getTime();
        });
        
        this.allPreviousFiles = allFiles.filter(file => {
          const fileDate = new Date(file.dateAdded);
          fileDate.setHours(0, 0, 0, 0);
          return fileDate.getTime() < today.getTime();
        });
        
        this.filteredNewFiles = [...this.allNewFiles];
        this.filteredPreviousFiles = [...this.allPreviousFiles];
    ...
    async fetchTreatmentPlans() {
      try {
        const response = await axios.get('/api/patient/my-treatment-plans', { withCredentials: true });
        return response.data.map(plan => ({
          id: plan.TreatmentPlanID,
          name: plan.FileName,
          type: plan.FileType,
          description: plan.Description || plan.FileName,
          dateAdded: plan.created_at || plan.UploadDate,
    ...
    async fetchDentalPhotos() {
      try {
        // Updated to use the correct endpoint from patientdentalphotos.vue
        const response = await axios.get('/api/patient/my-dental-photos', { withCredentials: true });
        return response.data.map(photo => ({
          id: photo.DentalPhotoID,
          name: photo.FileName,
          type: photo.FileType,
          description: photo.Description || photo.FileName || 'Dental Photo',
          dateAdded: photo.UploadDate || photo.created_at,
          url: null // Will be fetched when needed
        }));
      } catch (error) {
    ...
    async fetchForms() {
      try {
        const response = await axios.get('/api/patient/dental-forms', { withCredentials: true });
        return response.data.map(form => ({
          id: form.DentalFormID,
          name: form.FileName,
          type: form.FileType,
          description: form.Description || form.FileName,
          dateAdded: form.created_at || form.UploadDate,
          url: null // Will be fetched when needed
        }));
      } catch (error) {
    ...
    async fetchTreatmentHistory() {
      try {
        ...
        return response.data.map(appointment => ({
          ...
          dateAdded: `${appointment.AppointmentDate} ${appointment.AppointmentTime}`,
          dentist: appointment.DentistName,
          treatment: appointment.Services.length > 0 
            ? appointment.Services.map(s => s.ServiceName).join(', ') 
            : 'N/A',
          notes: appointment.PatientNote,
          description: `Treatment: ${appointment.Services.length > 0 
            ? appointment.Services.map(s => s.ServiceName).join(', ') 
            : 'N/A'}`,
    ...
    matchesSearch(file) {
      ...
      return (
        ...
        (file.dateAdded || '').toLowerCase().includes(query) ||
        (file.category || '').toLowerCase().includes(query) ||
        (file.type || '').toLowerCase().includes(query) ||
        (file.dentist || '').toLowerCase().includes(query) ||
        (file.treatment || '').toLowerCase().includes(query)
      );
    },
    
    applyAllFilters(files) {
      let filtered = [...files];
      
      if (this.searchQuery && this.isSearchActive) {
        filtered = filtered.filter(file => this.matchesSearch(file));
      }
      ...
      if (this.filter.category) {
        filtered = filtered.filter(file => file.category === this.filter.category);
      }
    ...
    updateDisplayedFiles() {
      this.filteredNewFiles = this.applyAllFilters(this.allNewFiles);
      this.filteredPreviousFiles = this.applyAllFilters(this.allPreviousFiles);
      this.applySorting();
    },
    ...
    applySorting() {
      ...
      const sortArray = (array) => {
        array.sort((a, b) => {
          let valueA = key === 'index' ? array.indexOf(a) + 1 : a[key];
          let valueB = key === 'index' ? array.indexOf(b) + 1 : b[key];
      ...
      sortArray(this.filteredNewFiles);
      sortArray(this.filteredPreviousFiles);
    },
    ...
    async handleFileAction(file) {
      try {
        // Set the preview file first to show loading state
        this.previewFile = { ...file };
        this.showPreviewModal = true;
        document.body.style.overflow = 'hidden';
        
        // If it's a treatment history item, no need to fetch a file
        if (file.sourceType === 'treatment-history') {
          return;
        }
        ...
        switch (file.sourceType) {
          case 'treatment-plan':
            endpoint = `/api/patient/download/treatment/${file.id}?inline=true`;
            break;
          case 'dental-photos':
            // Updated to use the correct endpoint from patientdentalphotos.vue
            endpoint = `/api/patient/download/dental/${file.id}?inline=true`;
            break;
          case 'forms':
            endpoint = `/api/patient/download/form/${file.id}?inline=true`;
            break;
          default:
        ...
        // Update the preview file with the URL
    ...
    async handleFileDownload(file) {
      try {
        // If it's a treatment history item, no file to download
        if (file.sourceType === 'treatment-history') {
          alert('Treatment history records cannot be downloaded as files.');
          return;
        }
        ...
        switch (file.sourceType) {
          case 'treatment-plan':
            endpoint = `/api/patient/download/treatment/${file.id}`;
            break;
          case 'dental-photos':
            // Updated to use the correct endpoint from patientdentalphotos.vue
            endpoint = `/api/patient/download/dental/${file.id}`;
            break;
          case 'forms':
            endpoint = `/api/patient/download/form/${file.id}`;
            break;
          default:
            throw new Error('Unknown file type');
        }
        ...
        link.href = url;
        link.download = file.name || `file_${file.id}.${file.type}`;
...
/* Modal Styles - Back to Original Design */
.modal-overlay {
position: fixed;
top: 0;
left: 0;

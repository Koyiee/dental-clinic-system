    ...
    <div class="header">
      ...
      <div class="text">
        ...
        <div class="breadcrumbs">
          ...
          <a class="current" href="/patientdentalphotos"><p>Dental Photos</p></a>
        </div>
      </div>
    </div>
    <hr color="#06693a">
    ...
    <div class="search-wrapper">
      <div class="search-container">
        <input 
          ...
          @keyup.enter="filterPhotos" 
          class="search-input"
          :class="{ 'has-reset': searchQuery && isSearchActive }"
        />
        <i class="bx bx-search search-icon" @click="filterPhotos"></i>
        <button 
          v-if="isSearchActive" 
          @click="resetSearch" 
          class="reset-button"
      ...
      <div class="table-wrapper">
        ...
        <table class="treatment-table desktop-table">
          ...
          <tbody class="container-treatment">
            <tr 
              v-for="(photo, index) in filteredDentalPhotos" 
              :key="photo.id"
              class="treatment-row"
            >
              <td style="padding-left: 25px;" class="truncate-cell">{{ index + 1 }}</td>
              <td class="truncate-cell">{{ photo.description || stripFileExtension(photo.name) || 'N/A' }}</td>
              <td class="truncate-cell">
                <div class="file-type-badge" :class="getFileTypeClass(photo.type)">
                  {{ photo.type || 'N/A' }}
                </div>
              </td>
              <td class="truncate-cell">{{ formatDate(photo.date) }}</td>
              <td class="action-cell">
                <button @click="handleFilePreview(photo)" class="view-details-btn" title="View Details">
                  <i class='bx bx-show'></i>
                </button>
                <button @click="handleFileDownload(photo)" class="download-btn" title="Download">
                  <i class='bx bx-download'></i>
                </button>
              </td>
            </tr>
            <tr v-if="filteredDentalPhotos.length === 0">
              <td colspan="5" class="no-data">No dental photos found.</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="mobile-cards">
          ...
          >
            <div class="card-header">
              <h4>{{ photo.description || stripFileExtension(photo.name) || 'N/A' }}</h4>
              <div class="file-type-badge" :class="getFileTypeClass(photo.type)">
                {{ photo.type || 'N/A' }}
              </div>
            </div>
            ...
            <div class="card-footer">
              <button @click.stop="handleFilePreview(photo)" class="action-button-mobile">
                <i class='bx bx-show'></i> View
              </button>
              <button @click.stop="handleFileDownload(photo)" class="action-button-mobile">
                <i class='bx bx-download'></i> Download
              </button>
            </div>
          </div>
          <div v-if="filteredDentalPhotos.length === 0" class="no-data-mobile">
            No dental photos found
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchDentalPhotos() {
      try {
          const response = await axios.get('/api/patient/my-dental-photos');
          this.dentalPhotos = response.data.map(photo => ({
          ...
          this.filteredDentalPhotos = [...this.dentalPhotos];
      } catch (error) {
          console.error('Error fetching dental photos:', error);
          this.$notify({
              type: 'error',
              title: 'Fetch Failed',
              text: 'Failed to fetch dental photos. Please try again.'
          });
          this.dentalPhotos = [];
          this.filteredDentalPhotos = [];
      }
    },
    matchesSearch(photo) {
      ...
      return (
        ...
        (this.stripFileExtension(photo.name) || '').toLowerCase().includes(query) ||
        (photo.type || '').toLowerCase().includes(query) ||
        (this.formatDate(photo.date) || '').toLowerCase().includes(query)
      );
    },
    applyAllFilters(photos) {
      let filtered = [...photos];
      
      if (this.searchQuery && this.isSearchActive) {
        filtered = filtered.filter(photo => this.matchesSearch(photo));
      }
    ...
    updateDisplayedPhotos() {
      this.filteredDentalPhotos = this.applyAllFilters(this.dentalPhotos);
      this.applySorting();
    },
    toggleFilterMenu() {
    ...
    applySorting() {
      ...
      this.filteredDentalPhotos.sort((a, b) => {
        let valueA = key === 'index' ? this.filteredDentalPhotos.indexOf(a) + 1 : a[key];
        let valueB = key === 'index' ? this.filteredDentalPhotos.indexOf(b) + 1 : b[key];
    ...
    async handleFilePreview(file) {
      try {
          const endpoint = `/api/patient/download/dental/${file.id}?inline=true`;
    ...
    async handleFileDownload(file) {
      try {
          const endpoint = `/api/patient/download/dental/${file.id}`;
          const response = await axios.get(endpoint, { responseType: 'blob' });
          ...
          link.href = url;
          link.download = file.name || `dental_photo_${file.id}.${file.type}`;

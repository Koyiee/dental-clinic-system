...
async updateBilling() {
  try {
    ...
    for (const [groupKey, groupServices] of Object.entries(serviceGroups)) {
      ...
      const payload = {
        TotalAmount: this.selectedBilling.TotalAmount,
        ...
        services: groupServices.map(service => {
          ...
          return serviceAvailedIds.map(id => ({
            ...
            ServiceName: service.ServiceName,
            Cost: Number(service.Cost) / serviceAvailedIds.length,
    ...
    // Force reactive update by replacing allBillings
    this.allBillings = this.allBillings.filter(b => !String(b.BillingID).startsWith(baseBillingID));
    ...
    if (hasInsuranceService) {
      ...
      this.activeTab = 'unpaid'; // Default back to unpaid if no conditions met
    }

    // Fetch updated billings to ensure consistency
    await this.fetchBillings();

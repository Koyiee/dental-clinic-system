    ...
    public function getPatientLedger(Request $request, $patientId = null, $returnBalanceOnly = false)
    {
        ...
        try {
            ...
            foreach ($appointments as $appointment) {
                ...
                // Add appointment entries (debit for services)
                foreach ($servicesAvailed as $serviceAvailed) {
                ...
                if ($appointment->billing) {
                    ...
                    if ($discount > 0) {
                        $ledgerEntries[] = [
                            ...
                            'TransactionDate' => date('Y-m-d H:i:s', strtotime($transactionDate . ' +1 minute')),
                    ...
                    // Handle adjustments based on UpdatedPrice
                    foreach ($servicesAvailed as $serviceAvailed) {
                        ...
                        if ($service->IsMultiVisit && $updatedPrice !== null && abs($updatedPrice - $serviceCost) > 0.01) {
                            $adjustmentAmount = $updatedPrice - $serviceCost;
                            ...
                            Log::info("Added price adjustment", [
                                'TreatmentProgressID' => $treatmentProgressId,
                    ...
                    if ($billing->payments) {
                        foreach ($billing->payments as $payment) {
                            ...
                            $paymentTransactionDate = strtotime($paymentDate) > strtotime($transactionDate)
                                ? $paymentDate
                                : date('Y-m-d H:i:s', strtotime($transactionDate . ' +3 minutes'));
            ...
            // Sort entries by TransactionDate

...
public function store(Request $request)
{
    \Log::info('Signup attempt started', [
        ...
        'raw_password' => substr($request->input('Password'), 0, 3) . '...'
    ]);
    ...
    try {
        ...
        $yearMonth = date('Ymd'); // e.g., 20250618 for June 18, 2025, 05:57 PM PST
        $attempts = 0;
        $maxAttempts = 10;
        $patientId = null;

        do {
            $randomNum = str_pad(mt_rand(0, 99999), 5, '0', STR_PAD_LEFT); // 5 random digits
            $patientId = $yearMonth . $randomNum;
            $exists = Patient::where('PatientID', $patientId)->exists();
            $attempts++;
        } while ($exists && $attempts < $maxAttempts);

        if ($attempts >= $maxAttempts) {
            throw new \Exception('Unable to generate a unique PatientID');
        }
        ...
        // Create the patient record with generated PatientID
        $patient = new Patient([
            'UserID' => $user->UserID,
            'PatientID' => $patientId, // Assign the generated ID
            'GuardianID' => null,
